...
CREATE TABLE IF NOT EXISTS ingestion_work_items (
    ...
    CONSTRAINT ingestion_work_items_state_claim_valid CHECK (
        ...
        OR (
            ...
            AND next_claim_at IS NULL
        )
        OR (
            state = 'PAUSED' AND outcome IS NULL
            AND claim_token IS NULL AND lease_until IS NULL
            AND next_claim_at IS NULL
        )
        OR (
...
BEGIN
    ...
    THEN
        IF NEW.claim_generation <> OLD.claim_generation THEN
            RAISE EXCEPTION 'retry/pause/resume/terminal transition must preserve claim generation'
                USING ERRCODE = '23514';
        END IF;
    END IF;
    ...
    IF OLD.state = 'PAUSED' AND NEW.state = 'PENDING' THEN
        IF NEW.reason_code IS NOT NULL OR NEW.next_claim_at IS NULL THEN
            RAISE EXCEPTION 'resumed work must clear pause reason and schedule next claim'
                USING ERRCODE = '23514';
        END IF;
    END IF;
