...
기존 원시 체결 부하 실험은 `CPI 55회 × SPY·QQQ·SMH·NVDA 4종목 = 220개 구간`입니다. 여기에 분석용 provider bar 경로를 추가해 2026-09-03 기준으로 `77개 발표 × 10종목` 전체를 실제 수집·저장했습니다. 원시 체결 replay 결과와 provider bar 결과는 서로 다른 계층이므로 건수를 섞지 않습니다.

| event type | 공식 기관 | manifest 범위 | 이벤트 수 | 시장 데이터 상태 |
| --- | --- | --- | ---: | --- |
| CPI | BLS | 2022-01-12~2026-08-12 | 55 | 10종목 1분·3분·5분·일봉 저장 완료 |
| EMPLOYMENT | BLS | 2026-01-09~2026-08-07 | 8 | 10종목 1분·3분·5분·일봉 저장 완료 |
| PCE | BEA | 2026-01-22~2026-08-26 | 9 | 10종목 저장 완료, 최신 발표 이후 일봉은 5거래일까지 확보 |
| FOMC | Federal Reserve | 2026-01-28~2026-07-29 | 5 | 10종목 1분·3분·5분·일봉 저장 완료 |

따라서 현재 catalog는 77개 발표이며 종목 universe 10개를 적용한 770개 발표-종목 분석 구간을 실행했습니다. 실행 manifest와 발표 시각 범위로 PostgreSQL을 재조회한 결과는 1분봉 117,566행, 3분봉 43,184행, 5분봉 26,883행입니다. 일봉은 이벤트별 선택 건수를 합치면 11,520행이지만 인접한 이벤트가 같은 거래일을 공유하므로 PostgreSQL 고유 행은 8,740행입니다.
...
| `SESSION_1MIN` | `T-60분`부터 `T+120분`에 시작하는 분까지, 최대 181개 | Alpaca SIP 1분봉 | 발표 직후 5·30·60분과 당일 후속 반응 분석 | 전체 실행·DB 117,566행 확인 |
| `DERIVED_3M_5M` | 같은 181분의 1분봉을 3분·5분 단위로 집계 | 저장된 1분봉 | 희소 구간을 보기 쉬운 분석 해상도 제공 | 전체 실행·DB 43,184행 / 26,883행 확인 |
| `DAILY_15_SESSIONS` | 발표 거래일을 포함해 이전 7거래일 + 이후 7거래일 | Alpaca SIP 일봉 | 발표 전 움직임과 1·3·7거래일 후 흐름 분석 | 이벤트별 11,520행 선택, DB 고유 8,740행 확인 |
...
분석용 두 구간은 `collect_market_event_context.py`가 같은 공식 일정과 종목 목록을 사용해 수집합니다. 계획상 최대치는 `SESSION_1MIN` 139,370행, 파생 3분봉 46,970행, 파생 5분봉 28,490행과 이벤트별 일봉 11,550행입니다. 실제 실행에서는 거래가 없던 분을 만들지 않아 각각 117,566행, 43,184행, 26,883행이 저장됐습니다. 3분봉은 `COMPLETE` 35,255행·`PARTIAL` 7,929행, 5분봉은 `COMPLETE` 20,205행·`PARTIAL` 6,678행입니다.

일봉 coverage는 770개 발표-종목 중 750개가 `이전 7 + 발표일 1 + 이후 7`을 충족했습니다. 나머지 20개는 수집 실패가 아닙니다.

- 2026-04-03 고용보고서 발표일은 Good Friday 휴장이라 10종목 모두 발표일 행이 없고 `7 + 0 + 7`입니다.
- 2026-08-26 PCE는 2026-09-03 실행 시점에 이후 거래일이 5일만 지나 10종목 모두 `7 + 1 + 5`입니다. 미래 2거래일은 생성하지 않았습니다.
...
원시 체결 대량 수집은 `FOMC → 고용 → PCE → 기존 CPI의 신규 6종목` 순으로 나눕니다. 분석용 bar 전체 실행 결과는 `data/archive/market-event-context-manifest.json`에 남으며, 이 원본 manifest는 Git에서 제외하고 비밀정보가 없는 요약만 evidence 문서에 공개합니다.
