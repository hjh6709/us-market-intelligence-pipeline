...
> **역사 자료:** 3차시 제출 시점의 점검표입니다. 현재 플랫폼 완료 여부나 target architecture의 증거로 사용하지 않습니다.
...
| Kafka JSON 예시와 Topic | `raw.market-sip.v1`, 상세 과제 문서의 합성 JSON | 완료 |
| 실제 데이터 Producer | `src/historical_market_replay.py` | 완료 |
| Kafka 100건 이상 | 2026년 7월 CPI 발표 구간의 NVDA Historical SIP 개별 체결 레코드 58,036건 발행 | 완료 |
| Producer·Consumer 건수 대조 | `58,036 = 58,036`, `src/kafka_trace_consumer.py` | 완료 |
| Spark 처리 코드 | `src/spark_sip_trade_batch.py` | 완료 |
| 처리 전·후 건수 | 입력 58,036, 오류·중복 0, 최종 121행 | 완료 |
...
| 저장 결과 재현·실제 행 로컬 확인 | `scripts/evidence/cpi_sip_kafka_spark_evidence.sql` | 완료 |
...
3. 실행 보고서에서 `Producer 58,036건 = Consumer 58,036건 = Spark 입력 58,036건`을 보여준다.
4. `58,036개 고유 체결 레코드 → 121개 1분봉 → 중복 0개`와 발표 전후 121분 coverage를 보여준다.
5. 로컬 PostgreSQL에 아래 SQL을 실행해 OHLCV 실제 행을 보여준다.
6. WebSocket은 Kafka까지만 확인한 선행 실험이고, 제출 결과는 CPI 발표 구간 Historical Trades API의 실제 체결을 동일 Kafka·Spark 경로로 재생한 것이라고 구분한다.
...
실제 가격 행을 공개하지 않는 것은 데이터가 없어서가 아니라 Alpaca 재배포 조건을 지키기 위한 선택이다. 공개 저장소에는 집계 결과, 시간 범위, 중복 검사와 재현 코드를 증거로 남긴다.
