...
| 발표 당일 분석 | 발표 60분 전~120분 후, 최대 181개 1분봉 | 최대 139,370행 | FOMC×TLT 181행 smoke 완료, 전체 수집 전 |
| 분석용 파생봉 | 같은 181분을 3분·5분 단위로 집계 | 최대 46,970 / 28,490행 | FOMC×TLT 61 / 37행 smoke 완료 |
| 중기 분석 | 이전 7거래일+발표일+이후 7거래일 | 최대 11,550 일봉 | FOMC×TLT 15행 smoke 완료, 전체 수집 전 |

따라서 6차시 제출에서 139,370행이나 11,550행을 실제 저장 건수라고 주장하지 않습니다. 수집 후에는 이벤트·종목별로 `before=7`, `event=1`, `after=7`인지 기록하고, 최신 발표처럼 이후 거래일이 아직 오지 않은 경우 `incomplete`로 구분합니다.
