...
> 이 문서는 실제 실행 결과와 남은 작업을 분리합니다. 실행하지 않은 기능은 완료로 표시하지 않습니다.
...
- 원시 경로는 CPI 55회 × 4종목의 실제 SIP 체결 **7,360,804건**을 처리해 1분봉 **22,260행**을 저장했습니다.
- 5차시 GCP 실행에서 중복으로 표시된 49건은 실제 중복이 아니라 거래소가 다른 체결의 `event_id` 충돌이었습니다. 식별키에 거래소를 포함한 뒤 전체 원본을 다시 처리한 현재 결과는 **실제 중복 0건**입니다.
- 분석용 경로는 CPI·고용·PCE·FOMC **77회 × 10종목**을 수집했습니다. 선택 범위에서 1분봉 **117,566행**, 3분봉 **43,184행**, 5분봉 **26,883행**, 일봉 고유 **8,740행**을 확인했습니다.
- 이번 보완에서는 분석용 수집을 Airflow의 **경제발표 1건 × 종목 1개** 작업으로 연결했습니다. FOMC 1회 × SPY·TLT 동일 입력 재실행 전후 모두 **588행**, 같은 hash, 중복 0건이었습니다.
...
| fallback 또는 alert | mock 503 후 DB alert `OPEN → RESOLVED` | [Airflow·alert 증거](evidence/sixth-assignment/README.md) |
...
동일한 규칙을 2022-01-12부터 2026-08-12까지 CPI 55회로 확대했습니다. 외부 API에 736만 건의 동시 부하를 준 것이 아니라, 미리 수집해 checksum을 확인한 220개 Parquet 파티션을 Kafka에 재생했습니다.

| 항목 | 기준 | GCP 부하 당시 | 식별키 수정 후 현재 정확성 결과 |
| --- | ---: | ---: | ---: |
| CPI 발표 / 종목 | 1회 / 4개 | 55회 / 4개 | 55회 / 4개 |
| 실제 SIP 체결 | 118,118 | 7,360,804 | 7,360,804 |
| Kafka 발행 / 수신 | 118,118 / 118,118 | 7,360,804 / 7,360,804 | 7,360,804 / 7,360,804 |
| Spark 입력 | 118,118 | 7,360,804 | 7,360,804 |
| 형식 오류 | 0 | 0 | 0 |
| event ID 충돌 / 실제 중복 | 0 / 0 | 49 / 측정 오류 | 0 / 0 |
| PostgreSQL 1분봉 | 472 | 22,260 | 22,260 |
| DB business key 중복 | 0 | 0 | 0 |
| 실행 시간 | 76.480초 | 1,690.250초 | 436.653초(로컬) |

5차시 GCP 출력의 49건은 같은 종목·거래 ID·시각이지만 거래소가 다른 별개의 체결이었습니다. 기존 식별키에 거래소가 없어 같은 거래로 오인했습니다.
...
수정 후 7,360,804건 전수 검사와 Kafka → Spark → PostgreSQL 전체 재실행에서 실제 중복 0건을 확인했습니다. 수정 전 결과 hash는 폐기했고 현재 정확성 기준 hash는 `0e40961df007996bcb812532a4049193`입니다. GCP와 로컬은 환경이 달라 실행 시간을 성능 우열로 직접 비교하지 않습니다.
...
`117,566`은 선택한 발표-종목 범위의 합입니다. PostgreSQL에는 이전 실험의 다른 구간도 있어 테이블 전체 `alpaca/sip/1m` 행 수와 같지 않습니다. 3분봉과 5분봉은 결측을 숨기는 대체물이 아니며 포함된 1분봉 수에 따라 `COMPLETE` 또는 `PARTIAL` coverage를 저장합니다.

## 3. 실패 원인과 탐지

### Spark heap 부족

7,360,804건 처리 중 검증 결과와 거래 조건 결과를 RAM에 `cache()`하면서 `Java heap space`가 발생했습니다. 입력 파일 전체가 6GB라는 뜻이 아니라, 여러 단계에서 재사용하는 중간 DataFrame이 Spark JVM heap을 넘었다는 뜻입니다. `DISK_ONLY`로 바꾼 뒤 같은 전체 범위를 끝까지 처리했습니다.
...
fixture는 장애 제어만 검증하며 실제 시장 데이터로 사용하지 않습니다. 검증되지 않은 archive를 사용하는 fallback은 미구현이고 현재는 검증 실패 시 닫힌 상태로 실패합니다.
...
원본 Parquet에 7,360,804건이 모두 남아 있어 Alpaca API를 다시 호출하지 않았습니다. 수정된 `event_id`로 Kafka부터 재실행해 Spark 입력·실제 중복·DB 결과를 다시 확인했습니다.

```text
Parquet 원본 → Kafka 재발행 → Spark 전체 재처리 → PostgreSQL Upsert
              7,360,804       실제 중복 0        22,260행·중복 0
```

### GCP DB 장애 복구

DB 컨테이너를 다시 시작하고 health check 후 실패한 기준 입력의 Kafka 재생부터 다시 실행했습니다. 기존 key에 Upsert해 최종 22,260행과 중복 0건이 유지됐습니다.
...
`market_context_backfill_pipeline`은 event type, 날짜 범위, symbols, feed, data cutoff을 입력받습니다. bar 배열은 XCom에 넣지 않고 식별자와 건수만 전달합니다. FOMC 2026-07-29 × SPY·TLT를 실제 Alpaca SIP와 로컬 PostgreSQL로 실행했습니다.
...
588행은 `2종목 × (181개 1분봉 + 61개 3분봉 + 37개 5분봉 + 15개 일봉)`입니다.

### 최종 확인 기준

1. `Parquet manifest = Kafka published = Consumer received = Spark input`
2. Spark는 거래소를 포함한 결정적 `event_id`로 동일 체결 재전송만 중복 판별
...
- [GCP 부하·복구 증거](evidence/load-recovery/README.md)
- [식별키 수정 후 전체 재실행 증거](evidence/pipeline-review/README.md)
