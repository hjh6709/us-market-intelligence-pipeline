...
Odd Lot 조건의 체결은 실제 거래이므로 원시 입력과 거래량·거래 건수에는 포함됩니다. 그러나 provider 규칙상 OHLC·VWAP 가격은 갱신하지 않습니다. 한 분의 모든 체결이 Odd Lot이면 가격을 만들 수 없어 `market_bars` 행을 생성하지 않습니다. 이는 수집 유실이 아니라 정상적인 무가격 구간입니다.
