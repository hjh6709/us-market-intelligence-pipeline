...
> 이번 과제에서는 한 번의 CPI 발표 구간에서 사용한 실제 주식 체결 118,118건을 기준으로 삼고, 동일한 범위를 CPI 발표 55회로 확대해 7,360,804건을 GCP에서 처리했습니다. 저장해 둔 실제 체결을 Kafka에 재생하고 Spark로 1분봉을 만든 뒤 PostgreSQL에 저장했습니다. DB 복구 후 최종 22,260행과 중복 0건을 확인했고, 별도의 동일 입력 재실행에서도 결과 hash가 유지되는 것을 확인했습니다.
...
현재 1분봉은 원시 결과를 검증하는 정본입니다. 희소한 구간의 분석 편의를 위해 1분봉에서 3분봉과 5분봉을 추가 파생하도록 구현했습니다. 1분봉을 삭제하거나 없는 가격을 채우지 않으며, 파생봉마다 포함된 원본 분 수와 coverage를 함께 기록합니다. 구간 전체에 1분봉이 하나도 없으면 파생봉 역시 만들지 않습니다.
...
GCP DB 장애·복구에서는 최종 시장 1분봉 22,260행과 DB business key 중복 0건을 확인했습니다. 별도로 수행한 로컬 동일 입력 재실행 검증에서도 처리 전후 22,260행과 중복 0건이 유지됐고, 결과 hash가 모두 `85ba8d3153a1bbbd6277f969ecce39d4`로 같았습니다. macro point-in-time 위반도 0건입니다.
...
원시 체결 부하 실험의 121분 범위는 이미 검증된 결과이므로 다시 바꾸지 않습니다. 서비스 분석 범위는 아래처럼 별도 계층으로 넓힙니다.

| 계층 | 범위 | 데이터 양(77회 × 10종목 계획) | 상태 |
| --- | --- | ---: | --- |
| Kafka·Spark raw 검증 | 발표 전후 60분, 최대 121분 | 원시 체결 수는 실제 수집 후 확정 | CPI 55회 × 4종목 완료 |
| 발표 당일 분석 | 발표 60분 전~120분 후, 최대 181개 1분봉 | 실제 117,566행 | 77회 × 10종목 DB 확인 완료 |
| 분석용 파생봉 | 같은 181분을 3분·5분 단위로 집계 | 실제 43,184 / 26,883행 | DB 확인 완료, coverage 저장 |
| 중기 분석 | 이전 7거래일+발표일+이후 7거래일 | 선택 11,520행, DB 고유 8,740행 | 750 complete / 20 known incomplete |

139,370행과 11,550행은 결측이 전혀 없다고 가정한 상한이며 실제 결과가 아닙니다. 전체 실행에서 일봉 20개 구간이 미완전했습니다. Good Friday 휴장인 2026-04-03 고용보고서 10종목은 `7/0/7`, 아직 이후 거래일이 5일만 지난 2026-08-26 PCE 10종목은 `7/1/5`로 기록했습니다.
