...
Spark는 원본에서 같은 `event_id`를 가진 체결 49건을 찾아 한 번만 반영했습니다. PostgreSQL은 `(symbol, bar_start, timeframe, source, feed)`를 고유키로 사용해 같은 입력을 다시 실행해도 새 행이 중복 추가되지 않게 했습니다.
...
따라서 22,260행이 이론상 최대치보다 작은 것은 곧바로 수집 실패를 뜻하지 않습니다. 원본 1분봉은 검증용 정본으로 유지하고, 희소 구간 분석에는 3분봉과 5분봉을 추가 생성해 포함된 원본 분 수와 coverage를 함께 저장하도록 구현했습니다. FOMC×TLT smoke 결과는 [`../multi-event-expansion/fomc-tlt-context.json`](../multi-event-expansion/fomc-tlt-context.json)에서 확인할 수 있습니다.
...
같은 입력을 다시 처리하기 전과 후의 OHLC·거래량·거래 건수·VWAP 결과 hash도 모두 `85ba8d3153a1bbbd6277f969ecce39d4`로 같았습니다. 즉, 행 수만 같은 것이 아니라 저장된 결과 내용도 같았습니다.
