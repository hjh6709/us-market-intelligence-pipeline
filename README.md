...
| PostgreSQL 고유 시장 봉 | 1m **323,126** / 3m **112,593** / 5m **70,090** / 1d **11,680행** |
...
기존 전체 실행의 일봉 coverage 2,020건 중 1,980건은 완전하고 40건은 이유가 확인된 불완전 구간으로 기록됐습니다. 이 분류는 PR #28 이전 계약의 실행 증거이며 새 collection/observed 계약으로는 아직 재검증하지 않았습니다.
...
FOMC 2026-07-29의 `SPY`, `TLT` smoke 후, 공식 발표 202회 전체를 실제 실행했습니다. 시장 DAG의 mapped task 202개는 종목별 work item 2,020개를 522.660초에 처리했습니다. 당시 기록은 1,980개 성공·40개 미제공·미해결 alert 0이지만 coverage corrective 이전 상태입니다. 실행 완료와 건수는 유효한 증거로 유지하되, 새 계약의 상태 분포라고 주장하지 않습니다.
...
- 새 coverage 계약으로 기존 202×10 work item 상태를 재검증하고, accepted input이 달라질 때만 분석 결과 재계산
